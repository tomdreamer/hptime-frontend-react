...
class Structuredetails extends Component {
  ...
  componentDidMount() {
   ...
   console.log( structureItem.sunday)
    let times="" 
    if(structureItem.monday || structureItem.tuesday || structureItem.wednesday || structureItem.thursday || structureItem.friday || structureItem.saturday || structureItem.sunday){
   ...
   console.log(times)
    ...
    render(){
      const { newstructureArray, open, structureItem, times } = this.state;
          ...
          <div className="card border-bottom-0">
              <div className="card-header" id="headingOne">
                {open ? (
                  ...
                  <Button
                    variant="secondary"
                    block
                    className="btn btn-secondary btn-sm back"
                  >
                    <Link to={'/map'}>
                      Retour
                    </Link>
              ...
              >
                <div aria-labelledby="headingOne" data-parent="#accordion">
                    <Card >
                        <Card.Body>
                            <Card.Title>Nom: {structureItem.name}</Card.Title>
                            <Card.Subtitle className="mb-2 text-muted">Type: {structureItem.type}</Card.Subtitle>
                            <br></br>
                            <Card.Text>
                              Horaires d'ouverture: {times}
                            </Card.Text>
                            <Card.Link href="tel:+33{structureItem.phoneNumber}">Tél: {structureItem.phoneNumber}</Card.Link>
                            
                            <address className="d-block p-2 bg-info text-white">
                                <strong>Adresse:</strong><br></br>
                                  {structureItem.streetNumber} {structureItem.roadType} {structureItem.streetName}<br></br>
                                  {structureItem.city},{structureItem.zipCode}<br></br>
                            </address>
                        </Card.Body>
                    </Card>
                </div>
